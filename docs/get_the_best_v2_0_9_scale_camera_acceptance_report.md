...
## 2026-05-21 沙盒与边缘平移调整

根据当前沙盒经营手感复查，继续调整办公室地图和镜头移动：

- 沙盒从 `16 x 10` 扩展为 `32 x 20`，保留单格建造面积不变。
- 边缘平移速度调整为 `84`，略高于 WASD 的 `72`，用于鼠标指针贴边浏览大地图。
- 中键旋转相机期间不触发边缘平移，避免旋转镜头时被自动拖走。
- 鼠标指针离开游戏窗口后不再触发边缘平移，避免窗口外操作导致镜头继续移动。
